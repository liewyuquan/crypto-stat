...
```{r return data, echo=FALSE, message=FALSE}
# Bitcoin daily return
BDReturn <- c(NA,diff(df_lag_m$BTC.USD.Close)/df_lag_m$BTC.USD.Close*100)
BDReturn <- BDReturn[-length(BDReturn)]
df_lag_m$BTC_USD_Day_Return <- BDReturn
# TSLA daily return
TDReturn <- c(NA,diff(df_lag_m$TSLA.Close)/df_lag_m$TSLA.Close*100)
TDReturn <- TDReturn[-length(TDReturn)]
df_lag_m$TSLA_price_Day_Return <- TDReturn
df_lag_m
```
...
y_value = df_lag_m$TSLA_price_lag_1
...
```{r lm model log, echo=FALSE}
x_value = df_lag_m$BTC_USD_Day_Return
y_value = df_lag_m$TSLA_price_Day_Return
fit=lm(data = df_lag_m, x_value~y_value)
summary(fit)
ggplot(df_lag_m, aes(x = x_value, y = y_value)) +
  geom_point() +
